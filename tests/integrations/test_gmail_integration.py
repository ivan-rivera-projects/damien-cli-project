...
from damien_cli.core import config 
...
def mock_service():
    ...
    service.users.return_value.messages.return_value.batchModify.return_value.execute.return_value = {} # Simulates 204 No Content
    service.users.return_value.messages.return_value.batchDelete.return_value.execute.return_value = {} # Simulates 204 No Content
    return service
...
def test_list_messages_success(mock_service):
    ...
    mock_service.users.return_value.messages.return_value.list.return_value.execute.return_value = expected_response
    result = gmail_integration.list_messages(mock_service, query_string="is:unread", max_results=5)
...
def test_list_messages_with_page_token(mock_service):
    ...
    mock_service.users.return_value.messages.return_value.list.return_value.execute.return_value = expected_response
    result = gmail_integration.list_messages(mock_service, query_string="is:read", max_results=3, page_token="start_token")
    mock_service.users.return_value.messages.return_value.list.assert_called_once_with(
...
def test_list_messages_no_query(mock_service):
    expected_response = {'messages': [], 'nextPageToken': None} # Ensure nextPageToken is expected
    mock_service.users.return_value.messages.return_value.list.return_value.execute.return_value = expected_response
...
def test_get_message_details_success(mock_service):
    ...
    mock_service.users.return_value.messages.return_value.get.return_value.execute.return_value = expected_message_data
    message_id_to_get = 'msg1'
    format_to_use = 'metadata'
    result = gmail_integration.get_message_details(mock_service, message_id_to_get, email_format=format_to_use)
...
def test_get_message_details_invalid_format_uses_metadata(mock_service, capsys):
    ...
    result = gmail_integration.get_message_details(mock_service, message_id_to_get, email_format='bad_format')
    mock_service.users.return_value.messages.return_value.get.assert_called_once_with(
        userId='me', id=message_id_to_get, format='metadata'
    )
    assert result == expected_message_data
    captured = capsys.readouterr()
    assert "Damien received an invalid format 'bad_format'. Using 'metadata'." in captured.out
...
def test_get_message_details_no_service(capsys):
    ...
    assert result is None
    captured = capsys.readouterr()
    assert "Damien cannot get message details: Gmail service not available." in captured.out
...
def test_get_label_id_system_label(mock_service):
    ...
    assert gmail_integration.get_label_id(mock_service, "UnReAd") == "UNREAD" # Test case insensitivity for system labels
...
def test_get_label_id_user_label_found_and_cached(mock_service):
    ...
    assert gmail_integration.get_label_id(mock_service, "mylabelone") == "Label_1" # Test case insensitivity for user labels
...
def test_batch_modify_message_labels_success(mock_service):
    ...
    with patch.object(gmail_integration, 'get_label_id', side_effect=lambda svc, name: f"ID_{name.upper()}") as mock_get_id:
        result = gmail_integration.batch_modify_message_labels(mock_service, message_ids, 
        ...
        expected_body = {
            'ids': message_ids,
            'addLabelIds': ['ID_NEWLABEL', 'ID_TRASH'], # Assuming get_label_id returns these
            'removeLabelIds': ['ID_OLDLABEL', 'ID_INBOX']
        }
        mock_service.users.return_value.messages.return_value.batchModify.assert_called_once_with(userId='me', body=expected_body)

def test_batch_modify_message_labels_unknown_label_name(mock_service, capsys):
    message_ids = ["id1"]
    with patch.object(gmail_integration, 'get_label_id', side_effect=lambda svc, name: "ID_Known" if name=="Known" else None) as mock_get_id:
        result = gmail_integration.batch_modify_message_labels(mock_service, message_ids, add_label_names=["Unknown", "Known"])
        
        assert result is True # Still true because one label was valid and API call was made
        expected_body = {'ids': message_ids, 'addLabelIds': ['ID_Known']}
        mock_service.users.return_value.messages.return_value.batchModify.assert_called_once_with(userId='me', body=expected_body)
        captured = capsys.readouterr()
        assert "Damien Warning: Label name 'Unknown' not found, skipping for 'add'." in captured.out

def test_batch_modify_message_labels_no_valid_labels(mock_service):
    message_ids = ["id1"]
    with patch.object(gmail_integration, 'get_label_id', return_value=None) as mock_get_id:
        result = gmail_integration.batch_modify_message_labels(mock_service, message_ids, add_label_names=["Unknown"])
...
def test_batch_modify_message_labels_api_error(mock_service):
    ...
    with patch.object(gmail_integration, 'get_label_id', return_value="ID_ANY"): # Ensure it tries to make the call
        result = gmail_integration.batch_modify_message_labels(mock_service, ["id1"], add_label_names=["Any"])
        assert result is False

def test_batch_trash_messages(mock_service):
    message_ids = ["id1", "id2"]
    with patch.object(gmail_integration, 'batch_modify_message_labels') as mock_batch_modify:
        gmail_integration.batch_trash_messages(mock_service, message_ids)
        mock_batch_modify.assert_called_once_with(mock_service, message_ids, 
...
def test_batch_mark_messages_read(mock_service):
    ...
    with patch.object(gmail_integration, 'batch_modify_message_labels') as mock_batch_modify:
        ...
        mock_batch_modify.assert_called_once_with(mock_service, message_ids, remove_label_names=['UNREAD'])

def test_batch_mark_messages_unread(mock_service):
    message_ids = ["id1"]
    with patch.object(gmail_integration, 'batch_modify_message_labels') as mock_batch_modify:
        gmail_integration.batch_mark_messages(mock_service, message_ids, mark_as='unread')
        mock_batch_modify.assert_called_once_with(mock_service, message_ids, add_label_names=['UNREAD'])
...
def test_batch_delete_permanently_success(mock_service):
    ...
    mock_service.users.return_value.messages.return_value.batchDelete.assert_called_once_with(userId='me', body=expected_body)
