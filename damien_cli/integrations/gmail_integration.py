...
_system_labels = ["INBOX", "SPAM", "TRASH", "UNREAD", "IMPORTANT", "STARRED", "SENT", "DRAFT", "CATEGORY_PERSONAL", "CATEGORY_SOCIAL", "CATEGORY_PROMOTIONS", "CATEGORY_UPDATES", "CATEGORY_FORUMS"]
...
def get_label_id(service, label_name: str) -> Optional[str]:
    ...
    if not _label_name_to_id_cache: # If cache is empty, populate it
        try:
            ...
            for lbl in labels:
                _label_name_to_id_cache[lbl['name'].lower()] = lbl['id'] # Store names lowercase for case-insensitive lookup
                _label_name_to_id_cache[lbl['id']] = lbl['id'] # Also allow passing ID directly
        except HttpError as e:
            click.echo(f"Damien: Error fetching labels: {e}")
            return None # Cannot resolve if label list fetch fails
    
    # Lookup in cache
    if label_name in _label_name_to_id_cache: # Direct lookup first (good for IDs or exact case names)
        return _label_name_to_id_cache[label_name]
    return _label_name_to_id_cache.get(label_name.lower()) # Fallback to lowercase lookup for names
...
def list_messages(service, query_string: str = None, max_results: int = 10, page_token: str = None):
    if not service:
    ...
    try:
        ...
        click.echo(f"Damien encountered an unexpected error while listing messages: {e}")
        return None
...
def get_message_details(service, message_id: str, email_format: str = 'metadata'):
    ...
    try:
        ...
        if email_format.lower() not in valid_formats:
            click.echo(f"Damien received an invalid format '{email_format}'. Using 'metadata'.")
            email_format = 'metadata'

        # click.echo(f"Damien is fetching details for message ID: {message_id} (format: {email_format})...")
        message = service.users().messages().get(userId='me', id=message_id, format=email_format).execute()
        return message
...
def batch_modify_message_labels(service, message_ids: list, add_label_names: list = None, remove_label_names: list = None): # Renamed params for clarity
    """
    ...
    if add_label_names:
        for name in add_label_names:
            ...
            if label_id:
                ...
                click.echo(f"Damien Warning: Label name '{name}' not found, skipping for 'add'.")
    ...
    if remove_label_names:
        for name in remove_label_names:
            ...
            if label_id:
                ...
                click.echo(f"Damien Warning: Label name '{name}' not found, skipping for 'remove'.")
    ...
    try:
        ...
        click.echo(f"Damien encountered an API error during batch label modification: {error}")
        return False
    except Exception as e:
        click.echo(f"Damien encountered an unexpected error during batch label modification: {e}")
        return False
...
def batch_trash_messages(service, message_ids: list):
    ...
    return batch_modify_message_labels(service, message_ids, 
...
def batch_mark_messages(service, message_ids: list, mark_as: str):
    ...
    if mark_as.lower() == 'read':
        ...
        return batch_modify_message_labels(service, message_ids, remove_label_names=['UNREAD'])
    elif mark_as.lower() == 'unread':
        click.echo(f"Damien preparing to mark {len(message_ids)} messages as unread.")
        return batch_modify_message_labels(service, message_ids, add_label_names=['UNREAD'])
    else:
...
def batch_delete_permanently(service, message_ids: list):
    ...
    try:
        ...
        click.echo(f"Damien encountered an API error during batch permanent deletion: {error}")
        return False
    except Exception as e:
        click.echo(f"Damien encountered an unexpected error during batch permanent deletion: {e}")
        return False
